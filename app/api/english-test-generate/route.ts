...
중요:
- 정답과 오답 모두 반드시 현재 제공된 해당 passage의 내용만 바탕으로 만드세요.
- 같은 원문 PDF의 다른 Part, 앞부분, 뒷부분에 나온 소재를 선택지에 섞지 마세요.
- 현재 passage가 3D 횡단보도에 관한 내용이라면, 다른 Part에 나온 양면 인쇄·재활용·에너지 절약 같은 소재를 선택지에 사용하면 안 됩니다.
- 오답도 현재 passage 안의 세부 소재나 논점을 왜곡해서 만드세요.
- 현재 passage에 등장하지 않는 별개의 소재를 끌어오지 마세요.
...
중요:
- 모든 선택지는 반드시 현재 passage 안의 내용과 직접 관련되어야 합니다.
- 같은 원문 PDF의 다른 Part에서 나온 소재를 오답으로 사용하지 마세요.
- 오답은 현재 passage의 일부 내용만 과도하게 강조하거나, 핵심 논지를 살짝 비틀어 만드세요.
- 현재 passage에 없는 독립적인 다른 주제를 끌어오지 마세요.
...
중요:
- 모든 선택지는 반드시 현재 passage의 내용만 이용하세요.
- 다른 Part의 소재나 사실을 선택지에 섞지 마세요.
- 오답은 현재 passage의 세부 내용만 취하거나, 인과관계를 바꾸거나, 핵심 메시지를 부분적으로 왜곡하는 방식으로 만드세요.
- 현재 passage와 무관한 소재를 넣어서 쉽게 제거할 수 있는 오답을 만들지 마세요.
...
즉 번호 뒤의 해당 어휘를 반드시 __단어__ 형태로 감싸서 반환하세요.
시스템 화면에서 이 표시를 실제 밑줄로 렌더링합니다.

정확히 하나만 문맥상 부적절해야 합니다.
나머지 네 단어는 문맥상 자연스러워야 합니다.

choices:
["① maintain", "② reduce", "③ significant", "④ ignore", "⑤ beneficial"]
