...
- 모든 대사는 실제 사람이 입으로 말할 법한 문장으로 작성한다.
- "~한다.", "~이다.", "~된다." 같은 딱딱한 설명체를 반복하지 않는다.
- 해설자, 전문가, 연구자, 과학자도 자연스럽게 말한다.
- 설명은 정확하되 실제 대화나 내레이션처럼 부드럽게 표현한다.
- 학생은 자연스러운 일상 구어체를 사용한다.
- 해설자는 짧고 리듬감 있는 내레이션을 사용한다.
- 질문, 반응, 감탄을 적절히 섞는다.
- 지나치게 유치하거나 과한 인터넷 말투는 사용하지 않는다.
